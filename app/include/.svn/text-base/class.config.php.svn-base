...
class Config {
    ...
    var $mysqltzoffset=0;
    var $config=array();
    var $idiotites_array_keys=array();
    var $idiotites_array_values=array();
    ...
    function Config($id) { 
        $this->load($id);
        $sql =' SELECT  * FROM '.IDIOTITES_TABLE; 
        
		
        if(($res=db_query($sql)) && db_num_rows($res))
		{
		   $this->idiotites_array_keys=array();
		   $this->idiotites_array_values=array();
           while( $row=db_fetch_array($res))
		   {
			   array_push($this->idiotites_array_keys,$row['idiotita_id']);	
			   array_push($this->idiotites_array_values,$row['name']); 	
		   } 
        }	
		/**/		
    }
