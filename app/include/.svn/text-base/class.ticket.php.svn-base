...
include_once(INCLUDE_DIR.'class.wordpressXMLRPC.php');
...
class Ticket{
    ...
    var $phone_mobile;
    var $yphresia;
    var $yphresia_dimos_id;

    var $legalname;  //string   Επωνυμία Επιχείρησης ,
    var $sexId;  // int   Φύλο {0=Δεν δηλώνω , 1=Ανδρας , 2=Γυναίκα }
    var $agerangeId; //int Ηλικιακή Ομάδα {0=δεν δηλώνω , 1=16-30 , 2=30-45 , 3=45-60 ,4=60+}
    var $educationId; //int Εκπαίδευση {0=δεν δηλώνω , 1=Λυκείου , 2=Πανεπιστημίου , 3=Μεταπτυχιακό , 4=Διδακτορικό }
    var $topicId2; //
    var $eventdate; //
    var $seEkthesi; //
    //-->Added by Stav
	...
	//<--Added by Stav
            ...
            $this->dimos_id =$row['dimos_id'];
            $this->tk =$row['tk'];
            $this->address =$row['address'];
            $this->website =$row['website'];
            $this->idiotita =$row['idiotita'];
            $this->phone_mobile =$row['phone_mobile'];
            $this->communication_type =$row['communication_type'];
            $this->publish =$row['publish'];
            $this->yphresia =$row['yphresia'];
            $this->yphresia_dimos_id =$row['yphresia_dimos_id'];
			
            $this->legalname =$row['legalname'];
            $this->sexId =$row['sexId'];
            $this->agerangeId =$row['agerangeId'];
            $this->educationId =$row['educationId'];
            $this->topicId2 =$row['topicId2'];
            $this->eventdate =$row['eventdate'];
            $this->seEkthesi =$row['seEkthesi'];
    ...
    function getHelpTopic() {
 	...
 	function getTopicId2_name($topicId2)
	{
			$sql2='SELECT * from dict_reasons_level2 WHERE id='.$topicId2;
			$res2=db_query($sql2);
			$info2=db_fetch_array($res2);
			return $info2['name']; 	
	}  
    ...
    //<-- Added s
	 ...
	 function isPublishedAccepted() {
	        return  ($this->publish==2);
	    }
	 
	function isPublishedRejected() {
	        return  ($this->publish==3);
	    }
    ...
    function getcommunication_type(){
    ...
    function getyphresia_dimos_id(){
        return $this->yphresia_dimos_id;
    }

    function getyphresia(){
        return $this->yphresia;
    }
    ...
    function getsexId(){
 	...
 	function getsexId_name($inID)
	{
		if ($inId==0) { return "Δεν επιθυμώ να δήλώσω";}
		if ($inId==1) { return "Ανδρας";}
		if ($inId==2) { return "Γυναίκα";}
	}  

    function getagerangeId(){
        return $this->agerangeId;
    }
 	function getagerangeId_name($inID)
	{
		if ($inId==0) { return "Δεν επιθυμώ να δήλώσω";}
		if ($inId==1) { return "16-30";}
		if ($inId==2) { return "30-45";}
		if ($inId==3) { return "45-60";}
		if ($inId==4) { return "60+";}			
	}  
    ...
    function geteducationId(){
 	...
 	function geteducationId_name($inID)
	{
		if ($inId==0) { return "Δεν επιθυμώ να δήλώσω";}
		if ($inId==1) { return "Απόφοιτος Λυκείου";}
		if ($inId==2) { return "Πανεπιστήμιακές Σπουδές";} 
		if ($inId==3) { return "Μεταπτυχιακές Σπουδές";}
		if ($inId==4) { return "Κάτοχος Διδακτορικού";}			 	
	}  
    ...
    function getseEkthesi(){
        return $this->seEkthesi;
    }

    function getseEkthesi_text(){
		if ($this->seEkthesi)
		{
        	return "Ναι";
		}
    ...
    //save publish status
    function savePublishStatus($newstatus){
        global $cfg;
        if ($newstatus==2)
		{
       					 $this->publish=$newstatus;
						 $tplId=$cfg->getDefaultTemplateId();
		                 $sql='SELECT ticket_publish_accepted_subj,ticket_publish_accepted_body FROM '.EMAIL_TEMPLATE_TABLE;
						 $sql.=' WHERE cfg_id='.db_input($cfg->getId());
						 $sql.=' AND tpl_id='.db_input($tplId);
                    if(($resp=db_query($sql)) && db_num_rows($resp) && list($subj,$body)=db_fetch_row($resp)){
                        ...
                        Sys::log(LOG_WARNING,'Template Fetch Error',"Unable to fetch 'publish_accept message' auto response template #$tplId");
                    }			
		}
        $sql= 'UPDATE '.TICKET_TABLE.' SET publish='.$newstatus.
              ' WHERE ticket_id='.db_input($this->getId());
		$wp=new wordpressXMLRPC();
		$wp->updateWordPressPost(db_input($this->getId()),$newstatus);
		$wp='';
        return (db_query($sql) && db_affected_rows())?true:false;
    }
    ...
    function replaceTemplateVars($text){
        ...
        $replace = array($this->getId(),
                         ...
                         $cfg->getBaseUrl());
        return preg_replace($search,$replace,mb_convert_encoding($text, "UTF-8","AUTO" ));
    }
    ...
    function postMessage($msg,$source='',$msgid=NULL,$headers='',$newticket=false){
        ...
        if(db_query($sql) && ($msgid=db_insert_id())) {
           $wp=new wordpressXMLRPC();
		   $wp->createWordPressComment_message($msgid);
		   $wp='';
    ...
    function postResponse($msgid,$response,$signature='none',$attachment=false,$canalert=true){
        ...
        if(db_query($sql) && ($resp_id=db_insert_id())):
           $wp=new wordpressXMLRPC();
		   $wp->createWordPressComment_response($resp_id);
		   $wp='';
   ...
   /*============== Functions below do not require an instance of the class to be used. To call it use Ticket::function(params); ==================*/
	...
	//<--Added by Stav
         $fields['lastname']  = array('type'=>'string',   'required'=>0, 'error'=>'Last Name required');
         $fields['dimos_id']  = array('type'=>'int',      'required'=>0, 'error'=>'Invalid dimos_id');
         $fields['tk']  = array('type'=>'string',   'required'=>0, 'error'=>'TK required');
         $fields['address']  = array('type'=>'string',   'required'=>0, 'error'=>'Last Name required');
         $fields['website']  = array('type'=>'string',   'required'=>0, 'error'=>'Website required');
         $fields['idiotita']  = array('type'=>'int',      'required'=>0, 'error'=>'Invalid Idiotita');
         $fields['phone_mobile']  = array('type'=>'string',   'required'=>0, 'error'=>'Last Name required');
         $fields['communication_type']  = array('type'=>'string',   'required'=>0, 'error'=>'Communication Type required');
         $fields['publish']  = array('type'=>'int',   'required'=>0, 'error'=>'Publish required');
         $fields['yphresia']  = array('type'=>'string',   'required'=>0, 'error'=>'Yphresia Type required');
         $fields['yphresia_dimos_id']  = array('type'=>'int',   'required'=>0, 'error'=>'yphresia_dimos_id required');
		 
        $fields['legalname']  = array('type'=>'string',   'required'=>0, 'error'=>'legalname required');
        $fields['sexId']  = array('type'=>'int',   'required'=>0, 'error'=>'sexId required');
        $fields['agerangeId']  = array('type'=>'int',   'required'=>0, 'error'=>'agerangeId required');
        $fields['educationId']  = array('type'=>'int',   'required'=>0, 'error'=>'educationId required');
        $fields['topicId2']  = array('type'=>'int',   'required'=>0, 'error'=>'topicId2 required');
        $fields['eventdate']  = array('type'=>'string',   'required'=>0, 'error'=>'eventdate required');
        $fields['seEkthesi']  = array('type'=>'string',   'required'=>0, 'error'=>'seEkthesi required');
    //-->Added by Stav
        ...
        if($var['topicId'] && ($topic= new Topic($var['topicId'])) && $topic->getId()) {
         ...
         if(!$errors){
             $sql='UPDATE '.TICKET_TABLE.' SET updated=NOW() '.
                  ',lastname='.db_input($var['lastname']).
                  ',dimos_id='.db_input($var['dimos_id']).
                  ',tk='.db_input($var['tk']).
                  ',address='.db_input($var['address']).
                  ',website='.db_input($var['website']).
                  ',idiotita='.db_input($var['idiotita']).
                  ',phone_mobile='.db_input($var['phone_mobile']).
                  ',communication_type='.db_input($var['communication_type']).
                  ',publish='.db_input($var['publish']).
                  ',yphresia='.db_input($var['yphresia']).
                  ',yphresia_dimos_id='.db_input($var['yphresia_dimos_id']).
                  ...
                  ',legalname='.db_input($var['legalname']).
                  ',sexId='.db_input($var['sexId']).
                  ',agerangeId='.db_input($var['agerangeId']).
                  ',educationId='.db_input($var['educationId']).
                  ',topicId2='.db_input($var['topicId2']).
                  ',eventdate='.db_input($var['eventdate']).
                  ',seEkthesi='.db_input($var['seEkthesi']).
    ...
    function create($var,&$errors,$origin,$autorespond=true,$alertstaff=true) {
       ...
       /* Coders never code so fully and joyfully as when they do it for free  - Peter Rotich */
        ...
        $fields=array();
        $fields['name']     = array('type'=>'string',   'required'=>0, 'error'=>'Name required');
        $fields['email']    = array('type'=>'email',    'required'=>1, 'error'=>'Απαιτείται έγκυρο email');
        $fields['subject']  = array('type'=>'string',   'required'=>1, 'error'=>'Subject required');
        $fields['message']  = array('type'=>'text',     'required'=>1, 'error'=>'Το μήνυμα είναι υποχρεωτικό');
        if(strcasecmp($origin,'web')==0) { //Help topic only applicable on web tickets.
            $fields['topicId']  = array('type'=>'int',      'required'=>1, 'error'=>'Ο λόγος επικοινωνίας είναι υποχρεωτικός');
        }elseif(strcasecmp($origin,'staff')==0){ //tickets created by staff...e.g on callins.
            $fields['deptId']   = array('type'=>'int',      'required'=>1, 'error'=>'Ο φορέας είναι υποχρεωτικός');
            $fields['source']   = array('type'=>'string',   'required'=>1, 'error'=>'Άγνωστο πρόβλημα');
            $fields['duedate']  = array('type'=>'date',    'required'=>0, 'error'=>'Invalid date - must be MM/DD/YY');
...
//<--Added by Stav
     ...
     //-->Added by Stav
        ...
        if(!$errors && BanList::isbanned($var['email'])) {
            $errors['err']='Άγνωστο λάθος. Αριθμός #403'; //We don't want to tell the user the real reason...Psssst.
...
//var_dump($errors);
        ...
        $sql=   'INSERT INTO '.TICKET_TABLE.' SET created=NOW() '.
                ...
                ',legalname='.db_input($var['legalname']?$var['legalname']:'').        
                ',sexId='.db_input($var['sexId']?$var['sexId']:'').        
                ',agerangeId='.db_input($var['agerangeId']?$var['agerangeId']:'').        
                ',educationId='.db_input($var['educationId']?$var['educationId']:'').        
                ',topicId2='.db_input($var['topicId2']?$var['topicId2']:'').        
                ',eventdate='.db_input($var['eventdate']?$var['eventdate']:'').        
                ',seEkthesi='.db_input($var['seEkthesi']?$var['seEkthesi']:'').        
                ',source='.db_input($source);
