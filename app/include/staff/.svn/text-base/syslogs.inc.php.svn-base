...
?>
<div class="msg">Καταγραφή Συστήματος</div>
<div id='filter' >
    ...
    <div style="padding-left:15px;">
        ...
        &nbsp;From&nbsp;<input id="sd" size=15 name="startDate" value="<?=Format::htmlchars($_REQUEST['startDate'])?>" 
            ...
            <select name='type'>
                ...
                <option value="Error" <?=($type=='Error')?'selected="selected"':''?>>Σφάλματα</option>
                <option value="Warning" <?=($type=='Warning')?'selected="selected"':''?>>Ειδοποιήσεις</option>
                <option value="Debug" <?=($type=='Debug')?'selected="selected"':''?>>Αποσφαλμάτωση</option>
            </select>
            &nbsp;&nbsp;
