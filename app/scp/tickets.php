...
if(!$errors && ($id=$_REQUEST['id']?$_REQUEST['id']:$_POST['ticket_id']) && is_numeric($id)) {
	...
	if (($_REQUEST['a']=='edit') &&  ( ($ticket->getPublish()==3) || ($ticket->getPublish()==1)) )
	{
    ...
    if($ticket && $ticket->getId()) {
        ...
        case 'assign':
           ...
           if(!$errors && $ticket->assignStaff($_POST['staffId'],$_POST['assign_message'])){
                ...
                //$email->send($staff->getEmail(),$subj,$alert); //commented out by stav, this line crashes for some reason, no time to check out why. user used was ypes
        ...
        case 'process':
            ...
            switch(strtolower($_POST['do'])):
                ...
                case 'publish_accept':
                    ...
                    if(!$thisuser->isadmin() && !$thisuser->canCloseTickets()){
                        $errors['err']='Perm. Denied. You are not allowed to Accept tickets.';
                    }else{
                        if($ticket->savePublishstatus(2)){
                            $msg='Ticket #'.$ticket->getExtId().' status set to Accepted';
                            $note='Ticket closed without response by '.$thisuser->getName();
                            $ticket->logActivity('Ticket Accepted ',$note);
                            //$page=$ticket=null; //Going back to main listing.
							$page='viewticket.inc.php';
							//header("Location: ".$_SERVER["HTTP_REFERER"]);
							//exit;
                        }else{
                            $errors['err']='Problems Accepting the ticket. Try again';
                        }
                    }
                ...
                case 'publish_reject':
                    if(!$thisuser->isadmin() && !$thisuser->canCloseTickets()){
                        ...
                        if($ticket->savePublishstatus(3)){
							//$ticket->close();
                            $msg='Ticket #'.$ticket->getExtId().' status set to CLOSED and Rejected';
                            $note='Ticket closed and Rejected without response by '.$thisuser->getName();
                            $ticket->logActivity('Ticket Closed and Rejected',$note);
                            //$page=$ticket=null; //Going back to main listing.
							$page='viewticket.inc.php';
							//header("Location: ".$_SERVER["HTTP_REFERER"]);
							//exit;
                        }else{
                            $errors['err']='Problems closing and Rejecting the ticket. Try again';
                        }
                    }
                    break;
                case 'reopen':
...
if($cfg->showAnsweredTickets()) {
    $nav->addSubMenu(array('desc'=>'Ανοιχτά ('.($stats['open']+$stats['answered']).')'
                            ,'title'=>'Ανοιχτά Μηνύματα', 'href'=>'tickets.php', 'iconclass'=>'Ticket'));
}else{
//by stav, old line below was: 
//     $nav->addSubMenu(array('desc'=>'Ανοιχτά ('.$stats['open'].')','title'=>'Ανοιχτά Μηνύματα', 'href'=>'tickets.php', 'iconclass'=>'Ticket'));
    if($stats['open'])
        $nav->addSubMenu(array('desc'=>'Ανοιχτά ('.($stats['open']-$stats['answered']).')','title'=>'Ανοιχτά Μηνύματα', 'href'=>'tickets.php', 'iconclass'=>'Ticket'));
    if($stats['answered']) {
        $nav->addSubMenu(array('desc'=>'Υπο Επεξεργασία ('.$stats['answered'].')',
                           'title'=>'Μηνύματα Υπο Επεξεργασία', 'href'=>'tickets.php?status=answered', 'iconclass'=>'answeredTickets')); 
    }
}

if($stats['assigned']) {
    if(!$sysnotice && $stats['assigned']>10)
        $sysnotice=$stats['assigned'].' αναθετημένα στον δικό σας χρήστη!';

    $nav->addSubMenu(array('desc'=>'Τα Μηνύματα μου ('.$stats['assigned'].')','title'=>'Αναθετημένα Μηνύματα',
                    'href'=>'tickets.php?status=assigned','iconclass'=>'assignedTickets'));
}

if($stats['overdue']) {
    $nav->addSubMenu(array('desc'=>'Εκπρόθεσμο ('.$stats['overdue'].')','title'=>'Εκπρόθεσμα Μηνύματα',
...
$nav->addSubMenu(array('desc'=>'Κλειστά Μηνύματα','title'=>'Κλειστά Μηνύματα', 'href'=>'tickets.php?status=closed', 'iconclass'=>'closedTickets'));


if($thisuser->canCreateTickets()) {
    $nav->addSubMenu(array('desc'=>'Νέο Μήνυμα','href'=>'tickets.php?a=open','iconclass'=>'newTicket'));    
}
