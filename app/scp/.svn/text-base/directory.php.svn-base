...
if ($thisuser->getGroupId()=='3') //if user is in the staff group
{
	$WHERE.=' AND staff.dept_id='.$thisuser->getDeptId();
}
//End Aded by Stav
...
<div align="left">
    ...
    <select name="dept">
			...
			if ($thisuser->getGroupId()=='3') //if user is in the staff group
			{
				$depts= db_query('SELECT dept_id,dept_name FROM '.DEPT_TABLE.' WHERE dept_id='.$thisuser->getDeptId());
			}
			//End Aded by Stav
