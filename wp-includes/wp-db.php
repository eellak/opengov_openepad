...
class wpdb {
	...
	function set_prefix( $prefix, $set_table_names = true ) {
		...
		if ( preg_match( '|[^a-z0-9_]|i', $prefix ) )
			return new WP_Error('invalid_db_prefix', /*WP_I18N_DB_BAD_PREFIX*/'Άκυρο πρόθεμα βάσης δεδομένων'/*/WP_I18N_DB_BAD_PREFIX*/);
	...
	function select( $db, $dbh = null) {
		...
		if ( !@mysql_select_db( $db, $dbh ) ) {
			$this->ready = false;
			$this->bail( sprintf( /*WP_I18N_DB_SELECT_DB*/'<h1>Αδύνατη η επιλογή βάσης δεδομένων</h1>
<p>Η σύνδεση με τον διακομιστή της βάσης έγινε (που σημαίνει ότι όνομα και συνθηματικό είναι σωστά) αλλά ήταν αδύνατο να επιλεγεί η βάση δεδομένων <code>%1$s</code>.</p>
<ul>
<li>Είστε σίγουροι ότι υπάρχει η βάση;</li>
<li>Το όνομα χρήστη <code>%2$s</code> έχει δικαιώματα να χρησιμοποιεί τη βάση <code>%1$s</code>;</li>
<li>Σε κάποια συστήματα το όνομα της βάσης έχει ως πρόθεμα το όνομα χρήστη, δηλ. έχει μορφή <code>username_%1$s</code>. Μήπως είναι εκεί το πρόβλημα;</li>
</ul>
<p>Αν δεν ξέρετε από βάσεις δεδομένων, <strong>επικοινωνήστε με τον φιλοξενητή σας</strong>. Αν το πρόβλημα επιμείνει, ζητήστε βοήθεια στο <a href="http://wordpress.org/support/">Φόρουμ υποστήριξης του WordPress</a>.</p>'/*/WP_I18N_DB_SELECT_DB*/, $db, $this->dbuser ), 'db_select_fail' );
			return;
		}
	}
	...
	function print_error( $str = '' ) {
		...
		if ( $caller = $this->get_caller() )
			$error_str = sprintf( /*WP_I18N_DB_QUERY_ERROR_FULL*/'Σφάλμα %1$s βάσης δεδομένων WordPress για αίτηση %2$s από %3$s'/*/WP_I18N_DB_QUERY_ERROR_FULL*/, $str, $this->last_query, $caller );
		else
			$error_str = sprintf( /*WP_I18N_DB_QUERY_ERROR*/'Σφάλμα %1$s βάσης δεδομένων WordPress για αίτηση %2$s'/*/WP_I18N_DB_QUERY_ERROR*/, $str, $this->last_query );
...
'/*/WP_I18N_DB_CONN_ERROR*/, $this->dbhost ), 'db_connect_fail' );
	...
	function get_row( $query = null, $output = OBJECT, $y = 0 ) {
		...
		if ( $output == OBJECT ) {
			...
			$this->print_error(/*WP_I18N_DB_GETROW_ERROR*/' $db->get_row(string query, output type, int offset) -- Ο τύπος εξόδου πρέπει να είναι ένα εκ των OBJECT, ARRAY_A, ARRAY_N'/*/WP_I18N_DB_GETROW_ERROR*/);
		}
	}
